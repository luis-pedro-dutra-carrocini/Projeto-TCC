...
$verifica_existe = $dados_redacao['titulo'];
...
// Obtendo o titulo antigo
$titulo_antigo = $dados_redacao['titulo'];
...
if (isset($_POST['enviarredacao'])){
  ...
  // Obtendo o titulo da redação
  $titulo = trim($_POST['txttitulo']);

  // verificando se o titulo não esta nulo
  if ($titulo == ""){

    // Emitindo mensagem de erro
		$script = "<script>alert('Erro: Não foi possivel enviar a redação. Título não pode ser nulo.');location.href='nova_redacao.php';</script>";
		echo $script;
		exit;
  }
  ...
  // Tipo anexo
    ...
    if (!empty($_FILES['arquivo']['name'])){
      ...
      // Verificando o tipo do arquivo
        ...
        // Emitindo mensagem de erro
				$script = "<script>alert('Erro: Não foi possivel cadastra Pergunta. Tipo de arquivo não permitido no anexo. Tipo: $tipo');location.href='alterar_redacaousu.php';</script>";
				echo $script;
				exit;
			}
      ...
      // Salvando o arquivo
        ...
        // Emitindo mensagem de erro
				$script = "<script>alert('Erro ao salvar o naexo.');location.href='alterar_redacaousu.php';</script>";
				echo $script;
				exit;
			}
		}
      ...
      // Emitindo mensagem de erro
			$script = "<script>alert('Erro: Anexo não selecionado.');location.href='alterar_redacaousu.php';</script>";
			echo $script;
			exit;
		}
  }else{
  ...
  $result = mysqli_query($conexao, "UPDATE tabela_redacoes SET texto_naocorrigido='".addslashes($textored)."',data_envio='$hoje', titulo='".addslashes($titulo)."', tema='".addslashes($tema)."', visualizacao=$visu, tipo=$tipored WHERE codigo=$codigo_red;");
...
<b>Tema:</b>
<br>
<input type="radio" name="tipotema"  id="enem" value="enem" <?php echo $radtemaenem; ?>>
<label for="tipotema">ENEM</label>
&nbsp;&nbsp;
<input type="radio" name="tipotema" id="professor" value="professor" <?php echo $radtemaprof; ?>>
<label for="tipotema">Elaborado por Professores</label>
&nbsp;&nbsp;
<input type="radio" name="tipotema" id="usuario" value="usuario">
<label for="tipotema">Elaborado por Usuários</label>
&nbsp;&nbsp;
<input type="radio" name="tipotema" id="pormim" value="pormim" <?php echo $radtemameu; ?>>
<label for="tipotema">Elaborado por mim</label>
<br><br>

<div id="divmeutema" style="display: <?php echo $mostemameu; ?>;">
<input type="text" name="txttema" style="width: 99%;" maxlength="300" Placeholder="Tema" id="txttema" value="<?php echo $temamos; ?>">
<br><br>
</div>
...
// Varivel para guardar se o tema foi encontrado
$temaprofencontrado = 0;
...
if ($codigo_temaprof == $codigo_tema){
  ...
  // Exibindo tema
  echo "<input type='radio' name='temaprof' value='".$codigo_temaprof."' checked>
  <label for='temaprof'>".$dadotemaprof['tema']."</label> - ".$dadotemaprof['ano']."<br>";

  // Ajustando o valor da varivel
  $temaprofencontrado = 1;
}else{

  // Se o tema não foi encontrado
  if ($temaprofencontrado == 0){

    // Exibindo tema checado
    echo "<input type='radio' name='temaprof' value='".$codigo_temaprof."' checked>
    <label for='temaprof'>".$dadotemaprof['tema']."</label> - ".$dadotemaprof['ano']."<br>";
  }else{

    // Exibindo o tema sem estar checado
    echo "<input type='radio' name='temaprof' value='".$codigo_temaprof."' >
    <label for='temaprof'>".$dadotemaprof['tema']."</label> - ".$dadotemaprof['ano']."<br>";
  }
}
}
...
if ($codigo_temaenem == $codigo_tema){
  ...
  // Exibindo tema
  echo "<input type='radio' name='temaenem' value='".$codigo_temaenem."' checked>
  <label for='temaenem'>".$dadotemaenem['tema']."</label> - ".$dadotemaenem['ano']."<br>";

  // Ajustando o valor da varivel
  $temaenemencontrado = 1;
}else{

  // Se o tema não foi encontrado
  if ($temaenemencontrado == 0){
    
    // Exibindo o tema checado
    echo "<input type='radio' name='temaenem' value='".$codigo_temaenem."' checked>
    <label for='temaenem'>".$dadotemaenem['tema']."</label> - ".$dadotemaenem['ano']."<br>";
  }else{

    // Exibindo o tema sem estar checado
    echo "<input type='radio' name='temaenem' value='".$codigo_temaenem."'>
    <label for='temaenem'>".$dadotemaenem['tema']."</label> - ".$dadotemaenem['ano']."<br>";
  }
}
}
...
echo "<input type='radio' name='temausuario' value='".$codigo_temausu."' checked>
<label for='temausuario'>".$dadotemausu['tema']."</label> - ".$dadotemausu['ano']."<br>";
}
...
<!-- Campo titulo da redação, com suas propriedades -->
<b>Título:</b>
<input type="text" name="txttitulo" style="width: 93%;" maxlength="50" required id="txttitulo" value="<?php echo $titulo_antigo; ?>">
<br><br>
...
<textarea cols="95" rows="15" style="width: 99%;" name="txtredacao" value="text"><?php echo $texto_antigo; ?></textarea>
<br><br>
</div>
...
// Verificando o tipo do tema
// Tema enem
var cheenem = document.querySelector("#enem");
cheenem.addEventListener("click", function() {
...
// Tema Professor
var cheprofessor = document.querySelector("#professor");
cheprofessor.addEventListener("click", function() {
...
// Tema elaborados por mim
var chepormim = document.querySelector("#pormim");
chepormim.addEventListener("click", function() {
divtemaenem.style.display = "none"; 
divtemaprof.style.display = "none"; 
...
// Tema Usuario
var cheusuario = document.querySelector("#usuario");
cheusuario.addEventListener("click", function() {
divtemaenem.style.display = "none"; 
divtemaprof.style.display = "none"; 
divmeutema.style.display = "none"; 
