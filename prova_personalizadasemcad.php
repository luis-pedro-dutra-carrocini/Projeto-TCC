...
// Verificando se pelo menos uma checbox foi selecionada
  ...
  // Redirecionando para a pagina gerar provas personalizadas, pois nenhuma check box foi selecionada
  header('location: gerar_simusim.php');
  exit;
  }
...
if($slq_qtques->num_rows <= $qtperguntas){
  ...
  $script = "<script>alert('Erro: Não foi possível gerar o simulado. Quantidade de questões exigidas, maior que a quantidade existente. Diminua a quantidade, ou selecione outras disciplinas.');location.href='gerar_simusim.php';</script>";
  echo $script;
  exit;
}
...
<!-- Botões para finalizar ou cancelar simulado -->
