...
if(!isset($_SESSION["senha_adm"])){
  ...
  if ($nivel == "admgeral" || $nivel == "adm"){
    ...
    // Obtendo os dados
    $consulta = mysqli_query($conexao,"SELECT * from tabela_disciplina order by disciplina;");
...
<div class="panel-heading" style="background-color:#363636;">
                ...
                <h3 class="panel-title" style="font-size: clamp(1em, 1em + 1vw, 1.5em);">Disciplinas</h3>
            ...
            <table class="table">
                ...
                <tr class="filters" style="background-color:#696969;">
                        ...
                        <th><input type="text" style="font-size: clamp(1em, 1em + 0.2vw, 1.5em);" class="form-control" placeholder="Disciplina" disabled></th>

                        <th><input type="text" style="font-size: clamp(1em, 1em + 0.2vw, 1.5em);" class="form-control" placeholder="Qtd. de Questões" disabled></th>
...
// Codigo
echo "<td>".$dado['codigo_disciplina']."</td>";

// Disciplina
echo "<td>".$dado['disciplina']."</td>";

$codigo_discipliana = $dado['codigo_disciplina'];

// Obtendo o numero de questões
$sql_qtper = mysqli_query($conexao,"SELECT * from tabela_pergunta where codigo_disciplina = $codigo_discipliana");
$qtperguntas = mysqli_num_rows($sql_qtper);


// Numero de questões cadastradas com ela
echo "<td>".$qtperguntas."</td>";


// Inserindo os botões alterar e excluir 
echo "<td><a href='alterar_disciplina.php?codigo=$dado[codigo_disciplina]' title='Alterar'><img src='img/alterar.png' height ='30px' width='30px' align='center'></a>";

if ($qtperguntas <1){
echo "&nbsp;&nbsp;<a href='confex_disciplina.php?codigo=$dado[codigo_disciplina]' title='Excluir'><img src='img/deletar.png' height ='30px' width='30px' align='center'></a></td>";
}else{
    echo "</td>";
}
}
...
$(document).ready(function(){
    ...
    $('.filterable .filters input').keyup(function(e){
        ...
        if ($filteredRows.length === $rows.length) {
            ...
            $table.find('tbody').prepend($('<tr class="no-result text-center"><td colspan="'+ $table.find('.filters th').length +'">Nenhuma Disciplina Encontrada</td></tr>'));
