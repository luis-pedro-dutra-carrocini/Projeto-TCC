...
if (!empty($_GET['codigo'])){
  ...
  $consulta = mysqli_query($conexao,"SELECT * from tabela_provasenem where codigo = $codigo;");
  ...
  $dadosprova=$consulta->fetch_array();

  $nomeprovafixo = $dadosprova['nomeprova'];

  $dia = $dadosprova['dia'];

  $ano = $dadosprova['ano'];

  $nomegabarito = $dadosprova['gabarito'];

  $nomeprova = $dadosprova['prova'];
  }
...
if(isset($_POST["adicionar"])){
    ...
    $ano = Addslashes(trim($_POST['anoprova']));

    $nomeprovafixo = trim($_POST['nomeprova']);
    ...
    //Verificando se a prova foi selecionada,
      ...
      // Obtendo o nome temporario do arquivo
			...
			if (!in_array ($tipoprova, $tipoprovaspermitidos)){
				...
				$script = "<script>alert('Erro: Não foi possivel alterar a Prova. Tipo de arquivo não permitido na Prova.');location.href='provas_cadastradas.php?codigo=".$codigo."';</script>";
				echo $script;
				exit;
			}
			...
			if (move_uploaded_file($nometemporarioprova, $caminho.$nomeprova)){
				...
				// Emitindo mensagem de erro
				$script = "<script>alert('Erro ao salvar a Prova.');location.href='provas_cadastradas.php?codigo=".$codigo."';</script>";
				echo $script;
				exit;
			}
    }else{
    ...
    //Verificando se a prova foi selecionada,
      ...
      // Obtendo o nome temporario do arquivo
			...
			if (!in_array ($tipogabarito, $tipogabaritospermitidos)){
				...
				// Emitindo mensagem de erro
				$script = "<script>alert('Erro: Não foi possivel cadastrar a Prova. Tipo de arquivo não permitido no Gabarito.');location.href='provas_cadastradas.php?codigo=".$codigo."';</script>";
				echo $script;
				exit;
			}
			...
			if (move_uploaded_file($nometemporariogabarito, $caminho.$nomegabarito)){
				...
				// Emitindo mensagem de erro
				$script = "<script>alert('Erro ao salvar o Gabarito.');location.href='provas_cadastradas.php?codigo=".$codigo."';</script>";
				echo $script;
				exit;
			}
    }else{
    ...
    $adicionar = mysqli_query($conexao, "UPDATE tabela_provasenem SET nomeprova='".addslashes($nomeprovafixo)."', prova='$nomeprova', gabarito='$nomegabarito', ano=$ano, dia='$dia' WHERE codigo=$codigo;");

    // Emitindo mensagem de erro
    $script = "<script>alert('Prova alterada com Sucesso');location.href='provas_cadastradas.php';</script>";
    echo $script;
    exit;
...
function voltar() {
    ...
    location.href='provas_cadastradas.php';
    ...
    <div class="box" align="left">
        ...
        <form action="" method="POST" name="f1" enctype="multipart/form-data">
            ...
            <fieldset>
                ...
                <input type="text" value= "<?php echo $nomeprovafixo;?>" required autofocus name="nomeprova" style="background-color: black; border: 2px solid white;; color: white; font-size: clamp(1em, 1em + 0.2vw, 1.5em); width: 99%;">
                <br><br>
                ...
                <br>
                <a href="provas_enem/<?php echo $nomeprova;?>"><?php echo $nomeprova;?></a>
                </div>
                ...
                <br>
                <a href="provas_enem/<?php echo $nomegabarito;?>"><?php echo $nomegabarito;?></a>
                <br><br>
                </div>
                </div>

                <b>Ano da Aplicação:</b>
				        <select name="anoprova">
                    <?php
          ...
          echo "<option value='".$ano."'>".$ano."</option>";
            ...
            $contano = $contano + 1;
                ...
                if ($dia != "1° Dia"){
                echo "<option value='1° Dia'>1° Dia</option>";
                }

                if ($dia != "2° Dia"){
                  echo "<option value='2° Dia'>2° Dia</option>";
                }
                ?>
                </select>
<br><br>
                ...
                <button type ="submit" id="alterar" name="adicionar">Alterar</button>
