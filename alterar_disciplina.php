...
if(!empty($_GET['codigo'])){
    ...
    if($result->num_rows > 0){
     ...
     $disciplinamostrar = $user_data['disciplina'];
...
$nomedisciplina = Addslashes(trim($_POST["nomeprova"]));

$dados = mysqli_query($conexao, "SELECT * FROM tabela_disciplina WHERE disciplina = '$nomedisciplina';");

if($dados->num_rows > 0){
    // Emitindo mensagem de erro
    $script = "<script>alert('Erro: Não foi possivel alterar a Disciplina. Disciplina já cadastrada.');location.href='alterar_disciplina.php';</script>";
    echo $script;
    exit;
}else{
    $adicionar = mysqli_query($conexao, "UPDATE tabela_disciplina SET disciplina='$nomedisciplina' WHERE codigo_disciplina=$codigo;");
    // Emitindo mensagem de erro
    $script = "<script>alert('Disciplina alterada com Sucesso');location.href='mostrar_disciplinas.php';</script>";
    echo $script;
    exit;
}
}
    ...
    <div class="box" align="left">
        ...
        <form action="" method="POST" name="f1" enctype="multipart/form-data">
            ...
            <fieldset>
                ...
                <br>
                <b>Nome da Disciplina:</b><br>
                <input type="text" value="<?php echo $disciplinamostrar;?>" required autofocus name="nomeprova" style="background-color: black; border: 2px solid white;; color: white; font-size: clamp(1em, 1em + 0.2vw, 1.5em); width: 99%;">
                <br><br>
