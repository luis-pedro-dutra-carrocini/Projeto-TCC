...
if($dados->num_rows > 0){
    ...
    $script = "<script>alert('Erro: Não foi possivel cadastrar a Disciplina. Disciplina já cadastrada.');location.href='adicionar_disciplina.php';</script>";
    echo $script;
    exit;
}else{
    $adicionar = mysqli_query($conexao, "INSERT into tabela_disciplina(disciplina) values('$nomedisciplina');");
    // Emitindo mensagem de erro
    $script = "<script>alert('Disciplina cadastrada com Sucesso');location.href='adicionar_disciplina.php';</script>";
    echo $script;
    exit;
}
}
