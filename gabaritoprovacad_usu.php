...
if(!empty($_GET['codigo'])){
    ...
    $select_dadppro = mysqli_query($conexao,"SELECT * FROM tabela_provas_usuario WHERE codigo = $codigo_prova;");
    $dados_prova = mysqli_fetch_assoc($select_dadppro);

    // Verificando se a prova realmente existe
    if (mysqli_num_rows($select_dadppro) < 1){

      // Redirecioanando para a pagina minhas provas, pois a prova não existe
