...
if(!isset($_SESSION["senha_adm"])){
  ...
  $consulta = mysqli_query($conexao,"SELECT * from tabela_redacoes where situacao = 1 and codigo_adm = $codigo_adm order by codigo;");
  ...
  if ($niveladm == "admgeral" || $niveladm == "adm" || $niveladm == "corretor"){
    ...
    // Redirecionando para a pagina adm, pois o nivl do adm não condiz com a pagina atual
...
<!-- Determinando caracteristicas basicas da pagina -->
...
<div class="panel-heading" style="background-color:#363636;">
                ...
                <h3 class="panel-title" style="font-size: clamp(1em, 1em + 1vw, 1.5em);">Redações Corrigidas</h3>
            ...
            <table class="table">
                ...
                <tr class="filters" style="background-color:#696969;">
                        ...
                        <th><input type="text" style="font-size: clamp(1em, 1em + 0.2vw, 1.5em);" class="form-control" placeholder="Data Envio" disabled></th>

                        <th><input type="text" style="font-size: clamp(1em, 1em + 0.2vw, 1.5em);" class="form-control" placeholder="Data Correção" disabled></th>
...
// Obtebdo data de envio e correção

$dten = $dado['data_envio'];

$dtco = $dado['data_correcao'];

$data_envio = date('d/m/Y',  strtotime($dten));

$data_correcao = date('d/m/Y',  strtotime($dtco));
...
<a href='corrigir_redacao.php?codigo=$dado[codigo]' title='Alterar'><img src='img/alterar.png' height ='30px' width='30px' align='center'></a>
...
$(document).ready(function(){
    ...
    $('.filterable .filters input').keyup(function(e){
        ...
        if ($filteredRows.length === $rows.length) {
            ...
            $table.find('tbody').prepend($('<tr class="no-result text-center"><td colspan="'+ $table.find('.filters th').length +'">Nenhuma readação Encontrada</td></tr>'));
