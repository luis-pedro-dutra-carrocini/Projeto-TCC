...
if(!isset($_SESSION["senha_adm"])){
  ...
  exit;
    ...
    if ($nivel == "admgeral" || $nivel == "adm"){
      ...
      // Redirecionando para a pagina adm, pois o nivel do adm n]ao condiz com a pagina atual
      header('location: pagina_adm.php');
      exit;
    }


    // Verificando se o nome chegou via FET
if(!empty($_GET['nome']))
{
    ...
    if($result->num_rows > 0){
        ...
        // Obtendo os dados do adm
        $dados = mysqli_query($conexao, "SELECT * FROM tabela_usuario WHERE nome = '$nome';");
        $dado=$dados->fetch_array();
    ...
    // Redirecionando para a pagina mostrar usuarios, pois o nome do usuario não chegou via GET
    header('location: mostrar_usuarios.php');
    exit;
}
}

// Quando clicar no botão banir
if (isset($_POST['btn_banir'])){

    // Obtendo o dado (motivo) do form 
    $motivo = $_POST['motivo_ban'];

    // Obtendo data e a ajustando
    date_default_timezone_set('America/Sao_Paulo');
    $hoje = date('Y/m/d');

    // Inserindo os dados na tabela usuarios banidos
    $inusu_ban = mysqli_query($conexao, "INSERT into tabela_usuarios_banidos(nome, email, motivo, data_ban, usuoupro) values ('$nome', '$email', '$motivo', '$hoje', 1)");

    // Deletando os dados da tabela provas usuarios
    ...
    $del_per = mysqli_query($conexao, "DELETE FROM tabela_provas_usuario WHERE codigo_usuario = $codigo_usuario;");
...
<!-- Definindo caracteristicas baicas como acentuação e titulo -->
<meta charset="UTF-8">
...
<!-- Legenda do form -->
<legend style="color:grey31; font-size:25px; font-weight: bold;">Banir Usuário</legend>
<br><br>
